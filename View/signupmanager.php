...
<section>
<form class="box" action="../Controller/ManagerController.php"onsubmit="return validation()" method="post">
      ...
      <input type="text"id="emails" name="email" placeholder="Email" />
...
function validation(){
      	...
      	var uid = document.getElementById('uname').value;
			...
			if(isNaN(mobileNumber)){
				document.getElementById('error_message').innerHTML =" ** mobile NUmber must write digits only not characters";
				return false;
			}
			...
			if(isNaN(nid)){
				document.getElementById('error_message').innerHTML =" ** nid must write digits only not characters";
				return false;
			}
			if(nid.length!=15){
